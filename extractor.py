...
class Extractor(Seq):
    ...
    def __init__(self, maf_file, annotations_file):
        ...
        self.coding = True
        self.annotations = Importer.get_lines_from_file(annotations_file)
        self.maf_file = Importer.get_lines_from_file(maf_file)
        self.tri_num = 0
    ...
    def extract_mutational_matrix(self, region_name, coding=True):
        ...
        mutations = copy.deepcopy(self.neutral_matrix)
        occurences = copy.deepcopy(self.trinucleotides_occurences)
    ...
    def analyse_maf_file(self, maf_file, exons):
        ...
        for progress, line in enumerate(maf_file):
            ...
            Extractor.print_progres(progress, file_len)
    ...
    def update_logs(self, region_name):

        self.logs += '\n' + region_name + '\n'
        self.logs += 'mutability:' + '\t' + str(self.mutation_number/float(self.tri_num)) + '\n'
        self.logs += 'mutations number:' + '\t' + str(self.mutation_number) + '\n'
        self.logs += 'trin number:' + '\t' + str(self.tri_num) + '\n'
